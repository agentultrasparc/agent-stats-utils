...
def get_badges(data):
    ...
    for category, ranks in {'cassandra_neutralizer': [100, 300, 1000], # special categories that only go up to gold
                            ...
                            'cryptic_memories_op': [1500, 15000, 10000000000000],
                           }.items():
        current = 'Locked'
        multiplier = 1
...
def summary(group='all', days=7):
    ...
    sql_before = f'''SELECT x.name, s.`date`, `level`, ap, explorer, discoverer,
                     ...
                     `cassandra-neutralizer`, hacker, translator,
                     sojourner, recruiter, magnusbuilder, recursions,
                     ...
                     operation_chronos, cryptic_memories_op
                     FROM (
